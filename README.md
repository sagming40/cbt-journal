...
| 단계 | 내용 | 상태 |
| :--- | :--- | :---: |
| **Phase 0** | 환경 세팅 (DB · 서버 · 프론트 뼈대) | ⬜ |
| **Phase 1** | 회원 관리 (가입 · 로그인 · JWT 인증) | ⬜ |
| **Phase 2** | 일기 작성 (왜곡 목록 조회 + 저장) | ⬜ |
| **Phase 3** | 일기 조회 (목록 · 상세 보기) | ⬜ |
| **Phase 4** | 대시보드 (통계 집계 + 차트) | ⬜ |
| **Phase 5** | 삭제 · 에러 처리 · UI 마무리 | ⬜ |
| **Phase 6** | 확장 기능 (소셜 로그인, 검색, 캘린더 등) | ⬜ |

> ✅ 각 단계는 `[DB → 백엔드 → 프론트엔드]`를 관통하는 하나의 완성된 기능 단위로 개발합니다.
