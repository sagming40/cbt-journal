...
## 2026-07-20 · Phase 1 프론트엔드 (로그인 페이지)

### 오늘 한 일
- `pages/Login.jsx` 작성 — 이메일/비밀번호 입력 폼, 로딩·에러 상태 관리
- `api/auth.js`의 `loginUser(email, password)` 연동 + `AuthContext`의 `login(token, user)` 연결
- `App.jsx`의 임시 로그인 라우트(`<div>로그인 페이지 (준비 중)</div>`)를 실제 `Login` 컴포넌트로 교체
- 브라우저 실제 테스트로 로그인 성공 → `/diaries` 리다이렉트 확인 완료
- `Login.css` 작성 — flexbox 기반 카드형 레이아웃, input focus·button hover/disabled 상태 스타일링

### 겪었던 이슈들

**1. `e.preventDefalut is not a function`**
`handleSubmit` 안에서 `e.preventDefault()`를 `e.preventDefalut()`로 오타(Default → Defalut) 낸 채 저장했다. 이 함수가 실행되자마자 TypeError로 멈춰버리면서, 그 아래 있던 `loginUser()` 호출이나 `login()`, `navigate()`까지 전부 실행되지 않았다. `preventDefault()`가 안 걸리니 브라우저의 기본 동작(폼 제출 → 새로고침)이 그대로 발동돼서, 겉보기엔 "로그인 버튼을 눌러도 아무 반응 없이 새로고침만 되는" 것처럼 보였다. 콘솔 에러가 새로고침과 동시에 사라져서 처음엔 원인을 못 찾았는데, DevTools 콘솔의 **"Preserve log"** 옵션을 켜서 새로고침 직전 로그를 붙잡아두고 나서야 정확한 줄과 원인을 특정할 수 있었다.

**2. `/login`이 아니라 `/`로 접속해서 다이어리 화면만 뜨는 것으로 착각**
`App.jsx`에 `<Route path="*" element={<Navigate to="/diaries" replace />} />`가 있어서, `/login`을 정확히 입력하지 않으면 무조건 `/diaries`로 튕기는 게 원래 의도된 동작이었다. 처음엔 "로그인 페이지가 안 들어가지는 버그"로 오해했지만, 실제로는 라우팅 로직이 정상 작동하고 있었던 것 — 주소창을 `localhost:5173/login`으로 정확히 쳐서 들어가니 바로 해결됐다.

### 오늘 배운 것 / 느낀 점
- 오타 하나(`Defalut`)가 함수 전체를 미실행 상태로 만들 수 있다는 걸 직접 겪어보니, 에러 스택트레이스에서 파일명과 줄 번호를 바로 확인하는 습관이 왜 중요한지 다시 체감했다.
- 브라우저가 새로고침되면 콘솔 로그도 같이 날아간다는 걸 몰랐는데, "Preserve log" 옵션으로 새로고침 전후 로그를 이어서 볼 수 있다는 걸 알게 됐다. 프론트엔드 디버깅에서 꽤 자주 쓰일 것 같다.
- 백엔드(Postman으로 확인)와 프론트엔드 중 어느 쪽 문제인지 먼저 좁히고 들어가는 순서가, 무작정 코드를 다시 읽는 것보다 훨씬 빠르게 원인을 찾게 해준다는 걸 다시 느꼈다.
- CSS(순수 문법)와 Tailwind 중 어떤 걸 먼저 배울지 고민했는데, Tailwind 클래스들이 결국 일반 CSS 속성의 축약형이라는 걸 이해하고 나니 "기초 원리 → 축약 도구" 순서로 가는 게 맞다고 판단. Tailwind 전환은 Phase 5~6 폴리싱 단계로 미루기로 결정.

### 다음에 할 일
- `Register.jsx`(회원가입 페이지) 구현 — 로그인 페이지와 구조 유사, 스타일링은 생략하고 로직 위주로
